...
class MoChatMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_skeleton(self, input_ids, attention_mask, past_key_values, labels, skeletons):
        ...
        pad_token_embedding = self.get_model().embed_tokens(torch.tensor([self.pad_token_id], device=self.device)).detach()
        vision_tower = self.get_vision_tower()
        skeleton_features = self.encode_skeletons(skeletons) #[16, 300, 1056] [16, 512]
        new_skeleton_features = []
        for i in range(len(skeleton_features)):
            cur_skeleton_features = self.get_model().mm_projector(skeleton_features[i])
            new_skeleton_features.append(cur_skeleton_features)
        
        max_length = max([emb.shape[0] for emb in new_skeleton_features])
        padded_new_skeleton_features = []
        for emb in new_skeleton_features:
            ...
            padded_new_skeleton_features.append(padded_emb)

        padded_new_skeleton_features = torch.stack(padded_new_skeleton_features)
        ...
        if vision_tower is None or skeletons is None or input_ids.shape[1] == 1:
            ...
            return input_ids, attention_mask, past_key_values, None, labels, padded_new_skeleton_features
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if skeleton_token_indices.numel() > 0:
                cur_skeleton_features = new_skeleton_features[cur_skeleton_idx]
                cur_skeleton_idx += 1
        ...
        for emb in new_input_embeds:
            emb = emb.to(self.device)  # 确保 emb 在目标设备上
            padding_length = max_length - emb.shape[0]
            padding_tensor = pad_token_embedding.repeat(padding_length, 1)
            padded_emb = torch.cat([emb, padding_tensor], dim=0)
        ...
        return None, attention_mask, past_key_values, padded_new_input_embeds, new_labels,padded_new_skeleton_features
